pipeline {
    ...
    stages {
        ...
        stage('Disable Signature in Policy') {
            steps {
                script {
                    try {
                        ...
                        // JSON body to disable the signature
                        def jsonBody = [enabled: false]
                        ...
                        if (checkJsonResponse.totalItems == 0) {
                            ...
                            echo "Signature '${signatureId}' has been successfully disabled for Policy ID '${policyId}'."
                        } else {
    ...
    post {
        success {
            script {
                try {
                    emailext(
                        subject: "F5 Signature Disable Success: Policy '${params.POLICY_NAME}'",
                        body: """
                            ...
                            <p>The signature has been disabled successfully.</p>
                        """,
                        to: '${EMAIL_RECIPIENTS}',
                        from: '${EMAIL_SENDER}',
                        mimeType: 'text/html'
        ...
        failure {
            script {
                try {
                    emailext(
                        subject: "F5 Signature Disable Failed: Policy '${params.POLICY_NAME}'",
                        body: """
